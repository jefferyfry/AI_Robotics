...
#add items in a large bag based on probability, fewer items for lower probability
probBag = []
minVal = min(w)
multiplier = 1.0/minVal
for i in range(N):
    number_items = int(w[i]*multiplier)
    for j in range (number_items):
        probBag.append(p[i])

#now sample bag N times to create p3
p3 = []
size = len(probBag)
for i in range(N):
    p3.append(random.choice(probBag))
