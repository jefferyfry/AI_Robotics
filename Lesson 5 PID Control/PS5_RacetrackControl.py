...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #case of x < r, measure distance to circle
        if x<radius:
            cte = sqrt((x-radius)**2. + (y-radius)**2.)-radius
        #case of x > 3r, measure distance to circle
        elif x>3*radius:
            cte = sqrt((x-3.*radius)**2. + (y-radius)**2.)-radius
        #case of y > r, in top section of the straight
        elif y > radius:
            cte = y - 2.*radius
        #case of y < r, in lower section of the straight
        elif y < radius:
            cte = -y
        else:
            print 'I forgot a case!'
        return cte
