...
#
def drawpaths(path,newpath,scale):
    import turtle
    ...
    for index in range(len(path)):
        turtle.pendown()
        turtle.setpos(path[index][0]*scale,path[index][1]*scale)

    turtle.color('green')
    for index in range(len(newpath)):
        if index==0:
            turtle.penup()
        else:
            turtle.pendown()
        turtle.setpos(newpath[index][0]*scale,newpath[index][1]*scale)
...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while keepOnKeepingOn:
        ...
        for index in range(len(newpath)):
            origPathPt = path[index]
            origPathPtX = origPathPt[0]
            origPathPtY = origPathPt[1]

            newPathPt = newpath[index]
            newPathPtXUpdate = newPathPtX = newPathPt[0]
            newPathPtYUpdate = newPathPtY = newPathPt[1]

            newPathPtXMinus1 = newpath[(index-1)%len(newpath)][0]
            newPathPtYMinus1 = newpath[(index-1)%len(newpath)][1]
            newPathPtXPlus1 = newpath[(index+1)%len(newpath)][0]
            newPathPtYPlus1 = newpath[(index+1)%len(newpath)][1]

            #alpha
            newPathPtXUpdate = newPathPtX + weight_data*(origPathPtX-newPathPtX) + weight_smooth*(newPathPtXPlus1+newPathPtXMinus1-2*newPathPtX)
            newPathPtYUpdate = newPathPtY + weight_data*(origPathPtY-newPathPtY) + weight_smooth*(newPathPtYPlus1+newPathPtYMinus1-2*newPathPtY)

            newpath[index] = [newPathPtXUpdate,newPathPtYUpdate]

            if abs(newPathPtXUpdate-newPathPtX) > tolerance or abs(newPathPtYUpdate-newPathPtY) > tolerance:
                keepOnKeepingOn=True
...
drawpaths(testpath1,smooth(testpath1),40)
