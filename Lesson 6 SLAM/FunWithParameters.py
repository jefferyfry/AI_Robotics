...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        #print 'index=',index," spath length=",len(spath)
        ...
        if u > 1.0 and index < len(spath) - 2:
            index += 1
        elif u > 1.0 and index >= len(spath) - 2:
            break
    ...
    return [myrobot.check_goal(goal), myrobot.num_collisions, myrobot.num_steps,avgErr]
...
def main(grid, init, goal, steering_noise, distance_noise, measurement_noise,
    ...
    #print 'weight_data=',weight_data," weight_smooth=",weight_smooth," p_gain=",p_gain," d_gain=",d_gain
...
def twiddle(grid, init, goal, steering_noise, distance_noise, measurement_noise,tol = 0.001): #Make this tolerance bigger if you are timing out!
    start_time = time.time()
    params = [0,0,0,0]
    dp = [1,1,1,1]
    best_err = main(grid, init, goal, steering_noise, distance_noise, measurement_noise,params[0],params[1],params[2],params[3])[3]
    n=0
    leastSteps = 1000
    while sum(dp) > tol:
        for index in range(len(params)):
            params[index]+=dp[index]
            res = main(grid, init, goal, steering_noise, distance_noise, measurement_noise,params[0],params[1],params[2],params[3])
            hitGoal = res[0]
            collisions = res[1]
            steps = res[2]
            err = res[3]
            if err<best_err and hitGoal == True and collisions == 0 :
#            if hitGoal == True and collisions == 0 and steps < steps:
                elapsed_time = time.time() - start_time
                if steps < leastSteps:
                    leastSteps = steps
                    print res,params," time=",elapsed_time/60.
                best_err = err
                dp[index]*=1.5
            else:
        ...
        n+=1
        #print 'n=',n
    return main(grid, init, goal, steering_noise, distance_noise, measurement_noise,params[0],params[1],params[2],params[3])
# ------------------------------------------------
#
